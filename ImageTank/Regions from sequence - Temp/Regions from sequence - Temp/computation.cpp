                 ...
                 const DTDictionary &parameters,DTMutableSet<DTImage> &output)
{
    ...
    DTMutableDoubleArray NegR2List(howMany*(timeback+1+timeforward));
    DTMutableDoubleArray NegRatioList(howMany*(timeback+1+timeforward));
    ...
    if (posInOutput!=timeList.Length()) {
        ...
        NegR2List = TruncateSize(NegR2List,posInOutput);
        NegRatioList = TruncateSize(NegRatioList,posInOutput);
    ...
    output.Finish(DTTable({
        ...
        CreateTableColumn("NegR2",NegR2List),
        CreateTableColumn("NegRatio",NegRatioList),
